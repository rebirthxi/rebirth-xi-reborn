...
{
    if (m_PBaseEntity->objtype != TYPE_PC || job >= MAX_JOBTYPE || job < 0)
    {
...
{
    if (m_PBaseEntity->objtype != TYPE_PC || JobID >= MAX_JOBTYPE || JobID < 0)
    {
...
void CLuaBaseEntity::addSimpleGambit(uint16 targ, uint16 cond, uint32 condition_arg, uint16 react, uint16 select, uint32 selector_arg, sol::object const& retry)
{
    ...
    if ((target == G_TARGET::SELF && (condition != G_CONDITION::HAS_TOP_ENMITY && condition != G_CONDITION::NOT_HAS_TOP_ENMITY)) || target == G_TARGET::PARTY)
