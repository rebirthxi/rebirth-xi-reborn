                ...
                if (sql->SetAutoCommit(false) && sql->TransactionStart())
                {
                    ...
                    if (ret != SQL_ERROR && sql->NumRows() > 0 && sql->NextRow() == SQL_SUCCESS)
                    {
                        ...
                        if (PItem)
                        {
                            ...
                            PItem->aug_src.augment_item_src = sql->GetIntData(6);
                            PItem->aug_src.aug0_src = sql->GetIntData(7);
                            PItem->aug_src.aug0_min = sql->GetIntData(8);
                            PItem->aug_src.aug0_max = sql->GetIntData(9);
                            PItem->aug_src.aug1_src = sql->GetIntData(10);
                            PItem->aug_src.aug1_min = sql->GetIntData(11);
                            PItem->aug_src.aug1_max = sql->GetIntData(12);
                            PItem->aug_src.aug2_src = sql->GetIntData(13);
                            PItem->aug_src.aug2_min = sql->GetIntData(14);
                            PItem->aug_src.aug2_max = sql->GetIntData(15);
                            PItem->aug_src.aug3_src = sql->GetIntData(16);
                            PItem->aug_src.aug3_min = sql->GetIntData(17);
                            PItem->aug_src.aug3_max = sql->GetIntData(18);
...
void SmallPacket0x04E(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    if (PChar->m_GMlevel == 0 && PChar->loc.zone->GetType() != ZONE_TYPE::CITY)
    {
...
void SmallPacket0x085(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    if ((PItem != nullptr) && ((gil != nullptr) && gil->isType(ITEM_CURRENCY)))
    {
        ...
        if( PItem->getID() == 16481 && PItem->aug_src.augment_item_src == 16481 ) // yat +1s
        {
            ...
            sql->Query(fmtQuery, PChar->id, "AugPoints", aug_points+1, aug_points+1);
